...
def _run_generate_evaluate(
    ...
    generator.run(run_id, db, config, llm_client)
    ...
    for theme in themes:
        ...
        with _ThemeContext(theme_id):
            while True:
                ...
                if feedback:
                    generator.refine(
                        run_id, db, config, llm_client, theme_id, feedback
                    )
                else:
                    logger.warning(
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="AI News Pipeline — nightly batch scraper and content generator.",
    )
    parser.add_argument(
        "--config",
        required=True,
    ...
    parser.add_argument(
        "--init-db",
        action="store_true",
        help="Initialize the database schema, then exit.",
    )
    parser.add_argument(
        "--log-file",
        default="pipeline.log",
        help="Path for the structured JSON log file (default: pipeline.log).",
    )
    return parser
